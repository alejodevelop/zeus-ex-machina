...
- The scene exposes gameplay debug state so local agent validation can read player position, movement state, walk bounds, and dash status.

## Files
- `src/game/movement.ts` - pure movement helpers for direction normalization, delta-based movement, dash state, and bounds clamping.
- `src/game/movement.test.ts` - unit coverage for diagonal normalization, speed-by-delta movement, dash rules, and bounds clamping.
- `src/game/scenes/GameScene.ts` - room layout, input wiring, player spawn, bounded movement loop, dash queue handling, and debug-state provider.
