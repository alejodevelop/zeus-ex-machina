...
## Summary
- `GameScene` is now an oiling-only prototype room for validating the gear-service loop in isolation.
...
- Debug snapshots and the dev HUD expose the current oiling objective, active gear, oil charges, and grind timer.
...
- `src/game/scenes/GameScene.ts` - oiling sandbox room wiring, interaction targeting, objective banner, and gear/pump presentation.
